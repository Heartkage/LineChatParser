...
for name in names:
    person = Person(name, line.GetTotalChatCount(name), line.GetMessageCount(name), line.GetTotalWordCount(name),line.GetSendStickerCount(name), line.GetSendPictureCount(name), line.GetSendVideoCount(name), line.GetSendVoiceCount(name), line.GetSendLinkCount(name))
...
for name in names:
    ...
    print("    {} 貼圖({:.0%}), {} 照片({:.0%}), {} 影片({:.0%}), {} 語音訊息({:.0%}), {} 網址連結({:.0%})".format(person.stickerCount, chatRoom.GetStickerRatio(name), person.pictureCount, chatRoom.GetPictureRatio(name), person.videoCount, chatRoom.GetVideoRatio(name), person.voiceCount, chatRoom.GetVoiceRatio(name), person.linkCount, chatRoom.GetLinkRatio(name)))
print("")
